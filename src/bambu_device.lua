...
function BambuDevice:connect()
  ...
  client:on("connect", function()
    ...
    pcall(function()
        ...
        -- Initialize printDisplayExtended
        self.device:emit_event({
    ...
    pcall(function()
        ...
        self.device:emit_event({
            attribute_id = "coolingFanSpeed",
            capability_id = "schoolheart47510.fansDisplayNum",
            component_id = "others",
            state = { value = 0 }
        })
        self.device:emit_event({
            attribute_id = "auxFanSpeed",
            capability_id = "schoolheart47510.fansDisplayNum",
            component_id = "others",
            state = { value = 0 }
        })
        self.device:emit_event({
            attribute_id = "chamberFanSpeed",
            capability_id = "schoolheart47510.fansDisplayNum",
            component_id = "others",
            state = { value = 0 }
        })
        ...
        -- V1.1.48: Use profile check instead of get_component (which creates crash)
...
function BambuDevice:_handle_message(msg)
    ...
    if print_data.ams then
        ...
        if print_data.ams.ams_exist_bits == "1" then
            ...
            log.info("DEBUG: AMS Keys: " .. ams_keys)
    ...
    local status, err = pcall(function()
        ...
        log.info("DEBUG: Checking Fan Keys: Cooling=" .. tostring(print_data.cooling_fan_speed) .. 
    ...
    if print_data.gcode_state then
        log.info("DEBUG: Received gcode_state: " .. tostring(print_data.gcode_state))
    end
    if print_data.mc_print_stage then
        log.info("DEBUG: Received mc_print_stage: " .. tostring(print_data.mc_print_stage))
    end
    ...
    if self.last_gcode_state then
      ...
      if status ~= self.last_emitted_status then
        ...
        if capabilities["schoolheart47510.statusMessage"] then
            ...
            pcall(function()
                self.device:emit_event(capabilities["schoolheart47510.statusMessage"].message(enum_message))
            end)
        end
