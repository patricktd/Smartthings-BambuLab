    ...
    if self.last_gcode_state then
      ...
      if status ~= self.last_emitted_status then
        ...
        local success, err = pcall(function()
            ...
            -- Emit raw event to bypass capability wrapper issues
            local event = {
            ...
            -- Try emitting on the component directly if possible
            if self.device.profile and self.device.profile.components and self.device.profile.components.main then
                self.device.profile.components.main:emit_event(event)
            else
