...
local function device_init(driver, device)
  ...
  pcall(function()
      ...
      -- Initialize Temperatures to 0 to avoid NaN
      device:emit_component_event(device.profile.components.bed, capabilities.temperatureMeasurement.temperature({value = 0, unit = "C"}))
      device:emit_component_event(device.profile.components.extruder, capabilities.temperatureMeasurement.temperature({value = 0, unit = "C"}))
      
      -- Initialize AMS Info
      if capabilities["schoolheart47510.amsInfo"] then
          device:emit_event(capabilities["schoolheart47510.amsInfo"].value("Loading..."))
      end
      if capabilities["schoolheart47510.filamentType"] then
          device:emit_event(capabilities["schoolheart47510.filamentType"].value("-"))
      end
  end)
end
...
local bambu_driver = Driver("bambu-lab-driver", {
  capability_handlers = {
    [capabilities["schoolheart47510.bambuLightV3"].ID] = {
      ["on"] = handle_switch,
      ["off"] = handle_switch,
