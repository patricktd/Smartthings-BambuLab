...
function BambuDevice:connect()
  ...
  self.client:on("connect", function()
    ...
    pcall(function()
        self.device:emit_event({
            attribute_id = "value",
            capability_id = "schoolheart47510.layerZ",
            component_id = "main",
            state = { value = "--" }
        })
    end)
...
function BambuDevice:_handle_message(msg)
    ...
    if print_data.layer_num then
        local current_layer = print_data.layer_num
        local total_layer = print_data.total_layer_num or "?"
        local layer_str = string.format("%s/%s", current_layer, total_layer)
        
        local cap_layer = capabilities["schoolheart47510.layerZ"]
        if cap_layer then
            self.device:emit_component_event(self.device.profile.components.main, cap_layer.value(layer_str))
        end
    end
    ...
    if print_data.lights_report then
      if print_data.lights_report[1] and print_data.lights_report[1].node == "chamber_light" then
       ...
       self.device:emit_component_event(self.device.profile.components.others, capabilities["schoolheart47510.bambuLightV3"].lightState(mode))
      end
    end

    if print_data.bed_temper then
      -- log.info("DEBUG: Bed Temp received: " .. tostring(print_data.bed_temper))
      self.last_bed_temper = print_data.bed_temper
      
      local bed_val = tonumber(print_data.bed_temper) or 0
      -- Emit to new temperatures capability
      local cap_temp = capabilities["schoolheart47510.temperatures"]
      if cap_temp then
          self.device:emit_component_event(self.device.profile.components.main, cap_temp.bedTemp(string.format("%.1f °C", bed_val)))
      end
    end
    ...
    if self.last_gcode_state then
      ...
      if status == "RUNNING" then
         ...
         -- Check cache validity (timeout removed)
         if self.last_mc_print_stage then
            -- Removed timeout check to persist status like "Heatbed Preheating"
    ...
    if print_data.nozzle_temper then
        -- log.info("DEBUG: Nozzle Temp received: " .. tostring(print_data.nozzle_temper))
        local nozzle_val = tonumber(print_data.nozzle_temper) or 0
        -- Emit to new temperatures capability
        local cap_temp = capabilities["schoolheart47510.temperatures"]
        if cap_temp then
            self.device:emit_component_event(self.device.profile.components.main, cap_temp.extruderTemp(string.format("%.1f °C", nozzle_val)))
        end
    end
    ...
    -- AMS Info Parsing
    if print_data.ams and print_data.ams.ams then
        local ams_data = print_data.ams.ams
        -- Check if we have trays
        if ams_data[1] and ams_data[1].tray then
            local trays = ams_data[1].tray
            local active_tray_id = nil
            
            -- Try to find active tray from 'tray_now' (sometimes in print_data.ams or print_data)
            -- Or infer from 'id' if available. 
            -- For now, let's just format all trays or find the one being used if possible.
            -- Usually 'curr_stage_type' or similar might hint, but 'tray_now' is best.
            ...
            if print_data.ams.tray_now then
                active_tray_id = print_data.ams.tray_now
            elseif print_data.vt_tray and print_data.vt_tray.id then
                 active_tray_id = print_data.vt_tray.id
            end
            
            local ams_info_str = ""
            local current_filament = "Unknown"
            
            for _, tray in pairs(trays) do
                local id = tray.id
                local type = tray.tray_sub_brands or tray.tray_type or "Unknown"
                local color_hex = tray.tray_color or ""
                local color_name = color_map[string.upper(color_hex:sub(1,6))] or color_hex
                
                local info = string.format("Slot %s: %s (%s)", id, type, color_name)
                if ams_info_str == "" then
                    ams_info_str = info
                else
                    ams_info_str = ams_info_str .. "\n" .. info
                end
                
                -- Active Tray Logic
                -- If active_tray_id matches ID
                if active_tray_id and tostring(id) == tostring(active_tray_id) then
                    current_filament = type
                end
            end
            
            -- Fallback: If printing and we have a vt_tray (virtual tray), use that
            if current_filament == "Unknown" and print_data.vt_tray then
                 current_filament = print_data.vt_tray.tray_sub_brands or print_data.vt_tray.tray_type or "Unknown"
            end
            
            -- Fallback 2: If still unknown but we have trays, use the first one (often single color print)
            if current_filament == "Unknown" and #trays > 0 then
                current_filament = trays[1].tray_sub_brands or trays[1].tray_type or "Unknown"
            end

            -- Emit Events
            if capabilities["schoolheart47510.amsInfo"] then
                self.device:emit_event(capabilities["schoolheart47510.amsInfo"].value(ams_info_str))
            end
            
            if capabilities["schoolheart47510.filamentType"] then
                self.device:emit_event(capabilities["schoolheart47510.filamentType"].value(current_filament))
            end
        end
    end
end
