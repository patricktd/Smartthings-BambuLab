...
local function device_init(driver, device)
  ...
  if device.profile.components.others and capabilities["schoolheart47510.filamentType"] then
      device:emit_component_event(device.profile.components.others, capabilities["schoolheart47510.filamentType"].value("-"))
  end
...
local function device_info_changed(driver, device, event, args)
  ...
  -- Handle Profile Switching (A1 vs V2)
  if device.preferences.printerModel and device.preferences.printerModel ~= "BambuPrinterV2" then
      -- If user selected a different model, try to switch profile
      -- Note: "BambuPrinterV2" is default.
      
      -- Check if we need to switch
      -- This assumes the preference value matches the profile name defined in YAML/Config
      -- Actually, we used options: "BambuPrinterV2" and "BambuPrinterA1" in YAML
      local target_profile = device.preferences.printerModel
      
      -- Only switch if current profile is different
      -- device.profile.id might be internal ID, request reference might be easier if we could check name
      -- But we can just try update, it is safe.
      print("DEBUG: Checking Profile Switch. Target: " .. target_profile)
      
      -- We need to know the *ID* or *Name* used in registration.
      -- Unlike init, here we pass the Name defined in metadata? 
      -- device:try_update_metadata({profile = "profileName"})
      
      -- Let's stick safe:
      device:try_update_metadata({profile = target_profile})
  elseif device.preferences.printerModel == "BambuPrinterV2" then
       device:try_update_metadata({profile = "BambuPrinterV2"})
  end
...
local bambu_driver = Driver("bambu-lab-driver", {
  capability_handlers = {
    ...
    [capabilities["schoolheart47510.auxFan"].ID] = {
      ["setFanSpeed"] = function(driver, device, command)
          local client = active_devices[device.id]
          if client then
              client:handle_aux_fan_speed(command.args.speed)
          end
      end
    }
  },
